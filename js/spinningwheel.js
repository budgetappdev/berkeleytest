...
create: function () {
    ...
    for (l = 0; l < this.slotData.length; l += 1) {
        ...
        out = '';file://localhost/Users/robertospeich/Documents/BSE/www/icon@2x.png
        for (i in this.slotData[l].values) {
            if(this.slotData[l].values[i].indexOf('color') == -1){
                out += '<li>' + this.slotData[l].values[i] + '<' + '/li>';
            }else{
                var slotcontent = this.slotData[l].values[i].split('/');
                this.slotData[l].values[i] = slotcontent[1];
                //console.log(eval(slotcontent[0]));
                out += '<li style="background-color:'+eval(slotcontent[0])+';text-shadow:0px 1px 1px white">' + slotcontent[1] + '<' + '/li>';
            }
        }
        ul.innerHTML = out;
